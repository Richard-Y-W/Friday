...
```bash
jarvis smoke-run "MALDI AMR" --limit 100 --deep-read-limit 5
jarvis smoke-run "ESBL CRE surveillance" --limit 100 --deep-read-limit 5 --output-dir .jarvis/smoke-runs/esbl-cre
```
...
jarvis labels review --batch-id batch_...
jarvis labels export --batch-id batch_... --output ...
jarvis labels eval --batch-id batch_...
```
